// Parser
// This parser needs a lot more error handling. Eventually, it will
// implement the standard html5 algorithm.
...
impl Parser {
    ...
    fn parse_element(&mut self) -> dom::Node {
        ...
        assert!(self.consume_char() == '<');
        assert!(self.consume_char() == '/');
        assert!(self.parse_name() == tag_name);
        assert!(self.consume_char() == '>');
...
pub fn parse(source: String) -> dom::Node {
    ...
    if nodes.len() == 1 {
        ...
        return dom::element("html".to_string(), dom::AttrMap::new(), nodes);
    }
}
