...
fn hyphen() {
    ...
    match *t.consume_script_data_escape_start_state().first().unwrap() {
        Token::CharToken(c) => assert_eq!(c, '-'),
        _ => assert!(false),
    }
    match t.state {
        State::ScriptDataEscapeStartDashState => assert!(true),
        _ => assert!(false)
    }
}
...
fn anything_else() {
    ...
    let tokens = t.consume_script_data_escape_start_state();
