...
fn eof() {
    ...
    let tokens = t.consume_after_doctype_public_keyword_state();
...
fn whitespace() {
    ...
    match t.state {
        State::BeforeDOCTYPEPublicIdentifierState => assert!(true),
        _ => assert!(false)
    }
}
...
fn greater_than() {
    ...
    match *t.consume_after_doctype_public_keyword_state().first().unwrap() {
