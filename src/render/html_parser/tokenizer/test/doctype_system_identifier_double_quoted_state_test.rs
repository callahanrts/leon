...
fn eof() {
    ...
    let tokens = t.consume_doctype_system_identifier_double_quoted_state();
...
#[test]
// Encountering '"' should:
//   Change to the AfterDOCTYPESystemIdentifierState
fn double_quote() {
    let mut t = Tokenizer::new("\"");
    let tokens = t.consume_doctype_system_identifier_double_quoted_state();
    ...
    match t.state {
        State::AfterDOCTYPESystemIdentifierState => assert!(true),
        _ => assert!(false)
    }
}
...
fn nullchar() {
    ...
    data.system_identifier = Some(String::from("s"));
    ...
    match t.current_token() {
        Token::DoctypeToken(data) => {
            assert_eq!(data.system_identifier.unwrap(), String::from("s\u{FFFD}"));
        },
        _ => assert!(false)
    }
}
...
fn greater_than() {
    ...
    match *t.consume_doctype_system_identifier_double_quoted_state().first().unwrap() {
...
fn anything_else() {
    ...
    match t.current_token() {
        Token::DoctypeToken(data) => {
            assert_eq!(data.system_identifier.unwrap(), String::from("sa"));
        },
        _ => assert!(false)
    }
}
