...
impl<'a> LayoutBox<'a> {
    ...
    fn calculate_block_width(&mut self, containing_block: Dimensions) {
        ...
        let auto = Value::KeywordValue("auto".to_string());
        // let mut width = style.value("width").unwrap_or(auto.clone());
        let mut width = auto.clone();

        // Margin, border, and padding all have initial value 0.0
        let zero = Value::LengthValue(0.0, Unit::Px);
        ...
        if width != auto && total > containing_block.content.width {
            if margin_left == auto {
                margin_left = Value::LengthValue(0.0, Unit::Px);
            }
            if margin_right == auto {
                margin_right = Value::LengthValue(0.0, Unit::Px);
            }
        }
        ...
        match  (width == auto, margin_left == auto, margin_right == auto) {
            ...
            (false, false, false) => {
                margin_right = Value::LengthValue(margin_right.to_px() + underflow, Unit::Px);
            }

            // If exactly one size is auto, its used value follows from the equality
            (false, false, true) => { margin_right = Value::LengthValue(underflow, Unit::Px) }
            (false, true, false) => { margin_left = Value::LengthValue(underflow, Unit::Px) }

            // If width is set to auto, any other auto values become 0.
            (true, _, _) => {
                if margin_left == auto { margin_left = Value::LengthValue(0.0, Unit::Px) }
                if margin_right == auto { margin_right = Value::LengthValue(0.0, Unit::Px) }

                if underflow >= 0.0 {
                    // Expand width to fill the underflow
                    width = Value::LengthValue(underflow, Unit::Px);
                } else {
                    // Width can't be negative. Adjust right margin instead
                    width = Value::LengthValue(0.0, Unit::Px);
                    margin_right = Value::LengthValue(margin_right.to_px() + underflow, Unit::Px);
                }
            }
            ...
            (false, true, true) => {
                margin_left = Value::LengthValue(underflow / 2.0, Unit::Px);
                margin_right = Value::LengthValue(underflow / 2.0, Unit::Px);
            }
        }
    ...
    fn calculate_block_height(&mut self) {
        ...
        if let Some(Value::LengthValue(h, Unit::Px)) = self.get_style_node().value("height") {
